...
import { CommandConfig } from '../src/types/manifest'

describe('context', () => {
  const normalArgv = ['node', 'main.js', 'thisiscontent', '--filename', 'aa.json', '-f', 'json']
  const commandManifest: CommandConfig = {
    name: 'samplecmd',
  ...
  it('context.getArgv should return argv', () => {
    const context = new Context(normalArgv, commandManifest)
    context.push(normalHandlerManifest)
  ...
  it('context.validate should return false when argument does not passed', () => {
    const context = new Context(['node', 'main.js'], commandManifest)
    context.push(normalHandlerManifest)
    expect(context.validate()).toStrictEqual(false)
  })

  it('context.validate should return false when unknown option passed', () => {
    const context = new Context(['node', 'main.js', 'thisiscontent', '--aaa', 'bbb'], commandManifest)
    context.push(normalHandlerManifest)
    expect(context.validate()).toStrictEqual(false)
  })
  ...
  it('context.validate should return false when required option does not passed', () => {
    ...
    const context = new Context(['node', 'main.js', 'thisiscontent'], commandManifest)
    context.push(normalHandlerManifest)
    expect(context.validate()).toStrictEqual(false)
  })
